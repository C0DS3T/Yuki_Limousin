...
function createOnlineVisitorsCounterElement()
{
    ...
    if (existingElement === null) // Only creates a new element if does not exist yet.
    {
        ...
        button.style.cssText = 'z-index:100; position: fixed; bottom: 5px; right: 10px; background-color:transparent; -moz-border-radius:28px; -webkit-border-radius:28px; border-radius:28px; border:2px solid #000000; display:inline-block; cursor:pointer; color:#000000; font-family:Trebuchet MS; font-size:13px; padding:9px 15px; text-decoration:none;';
...
function createOnlineVisitorsIframe()
{
    ...
    iframe.style.cssText = 'top: 50%; left: 50%; width:50%; height:18em; margin-top: -9em; margin-left: -25%; -moz-border-radius:8px; -webkit-border-radius:8px; border-radius:8px; border:3px solid #000000; position:fixed; z-index:124;';
