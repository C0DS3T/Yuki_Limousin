...
class JE_IndeedFront {
	...
	function load_job () {
		...
		$query 	=	array('q' => $settings['q'] , 'l' => $settings['l'] , 'start' => ($paged - 1 ) * 10 , 'limit' => 10 );
//		var_dump($_REQUEST);
	...
	function add_script() {
		...
		if ( is_home() || is_search() || is_post_type_archive('job') || is_tax('job_category') || is_tax('job_type') || apply_filters( 'je_is_index_enqueue_script', false ) ){
			wp_enqueue_script('indeed' , plugin_dir_url( __FILE__).'/js/indeed.js' , array ('job_engine') ) ;
...
class JE_ImportView {
	...
	function plugin_scripts () {
		...
		wp_register_script('je_indeed', plugin_dir_url( __FILE__).'/je_indeed.js', array('jquery', 'et_backbone', 'et_underscore'));
