...
define('JERSSIMPORT_VERSION', "2.2.5");
class JE_RSS_Import {
	...
	*/
	 ...
	 * save job
	*/
	function rss_save_jobs ($job, $import_author) {
		...
		// wp_set_object_terms($result, (array)$job['job_category'], 'job_category');
		// wp_set_object_terms($result, (array)$job['job_type'], 'job_type');
	...
	?>	
		...
		<?php echo '
			<tr>
				...
				<td><a target="_blank" href="{{permalink}}"> {{ title }}></td>
				<td>{{url}}</td>
				<td>{{creator}}</td>
				<td>{{date}}</td>
			</tr>';
