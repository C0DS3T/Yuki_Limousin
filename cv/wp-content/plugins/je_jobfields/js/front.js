(function($){
	...
	var jobField = {
		...
		dpEdit : function(model, $jobinfo){			
			...
			// //check box
			// var items = $('#modal_edit_job .field-check');
			// _.each(items, function(e, i, list){
			// 	var field = $('#modal_edit_job .cfield-' + e.ID);					
			// 	if(e.value !== '' && e.value !== 0) {					
			...
			// add js for taxonomy
            // var check_fs = $('input[type=checkbox]');           
            // if(check_fs.length > 0){
	           //  var name = check_fs.attr('name');
	           //  	if(typeof name != 'undefined' ){
	           //      	name = name.replace("[]","");
			         //    var terms_checked = model.get(name);
			                
		          //       _.each(terms_checked,function(key){	                    
		          //           var temp = 'input[type=checkbox][value='+key+']';                   
