(function ($){
	...
	JobEngine.Views.Resume_Image = Backbone.View.extend({
		...
		saveFields : function (event) {
			...
			// event.preventDefault();
			// var view 		= this,
			// 	content 	= $( event.currentTarget ).find('input.attach_id').val(),
			// 	container 	= $( $( event.currentTarget ).closest('.module') ),
			// 	field_name	= container.attr('data-resume');
	...
	$(document).ready(function () {
		...
		for(var i = 0 ; i < fields.length ; i++ ) {
			...
			// 	new JobEngine.Views.Resume_Image( {el : $('#fields-'+fields[i]['name']) , model : resume , jobseeker : jobseeker , field : fields[i]});
			// }

			// if(fields[i]['type'] == 'file'){

			// 	new JobEngine.Views.Resume_File( {el : $('#fields-'+fields[i]['name']) , model : resume , jobseeker : jobseeker , field : fields[i]});
